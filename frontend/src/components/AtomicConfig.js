...
export default function AtomicConfig() {
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    drawAtomicDiagram(ctx, canvas.width, canvas.height);
  }, []);
...
function drawAtomicDiagram(ctx, W, H) {
  ...
  // ── Arrow λ: |e⟩ → |g⟩ (azul cian, bajando — campo cavidad) ──────
